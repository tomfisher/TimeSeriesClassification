...
public class JournalOfClassification2012_Bagnall {
/* ARMA MODELS
...
/**
 * NEW EXPERIMENT 11/7/12: HEART RATE. See class HeartRate in package timeseriesclassification for info
 * 
 */        
        public static void heartRateExample(Instances heart){
            ArrayList<String> names=new ArrayList<>();
                    Classifier[] c =HeartRate.setSingleClassifiers(names);
        OutFile res=new OutFile("C:\\Research\\Results\\HeartRate\\singleClassifiersTime.csv");
        try{
            for(int i=0;i<c.length;i++){
                System.out.print(" running classifier "+names.get(i));
                Evaluation e=new Evaluation(heart);
                e.crossValidateModel(c[i], heart, heart.numInstances(), new Random());
                res.writeLine(names.get(i)+","+e.correct()/heart.numInstances());
                System.out.println(" Acc = "+e.correct()/heart.numInstances());
...
//		Instances fftTest,fftTrain;
		...
		double euclidAcc=0,armaAcc=0, pacfAcc=0,acfAcc=0;
                String path="C:\\Users\\ajb\\Dropbox\\TSC Problems\\";
                double[] histAcc=new double[4];
                
		for(int i=0;i<TimeSeriesClassification.fileNamesLengthSorted.length;i++){
			System.out.println(" Running data set "+TimeSeriesClassification.fileNamesLengthSorted[i]);
                        
	//1. Load test/train split
			String base=path+TimeSeriesClassification.fileNamesLengthSorted[i]+"\\"
			+TimeSeriesClassification.fileNamesLengthSorted[i];
	...
	//2. Transform FFT, ARMA and Run Lengths
			...
			try{
				...
				euclidAcc=ClassifierTools.singleTrainTestSplitAccuracy(new kNN(1),train,test);
...
//				dtwAcc=ClassifierTools.singleTrainTestSplitAccuracy(c,train,test);
				...
				acfAcc=ClassifierTools.singleTrainTestSplitAccuracy(new IB1(),acfTrain,acfTest);
				pacfAcc=ClassifierTools.singleTrainTestSplitAccuracy(new IB1(),pacfTrain,pacfTest);
			...
			//4. Write to file
			of.writeLine(TimeSeriesClassification.fileNamesLengthSorted[i]+","+euclidAcc+","+acfAcc+","+pacfAcc+","+","+armaAcc+","+histAcc[0]+","+histAcc[1]+","+histAcc[2]+","+histAcc[3]);
		}
...
//Rerun with alternative n, manually hacked!	
	...
	public static void Experiment12_OtherClassifiers(String fileName)
//This method tests different classifiers on the ACF
	{
		//Generate a model
			int n=1000;
			int nosModels=200;
			...
			of.writeString("ACF,ARMA_Euclid,RL_Euclid,FFT,RL_Gower,RL_DTW, ACF,");
			System.out.print("ACF,ARMA,RL_Euclid,RL_Gower,RL_Likelihood,RL_DTW,");
                        for(String s:names){
			of.writeString("ACF_"+s+",");
			System.out.print("ACF_"+s+",");
			...
			{
				paras=generateValidModel(10,20);
				double[] acc=AR_Classification(paras,c,n,100);
				of.writeString(m+","+n+",");
				System.out.print(m+","+n+",");
				for(int i=0;i<acc.length;i++){
					of.writeString(acc[i]+",");
	...
	public static void main(String[] args){
            Experiment12_OtherClassifiers("C:\\Users\\ajb\\Dropbox\\Results\\JoC2012 Runlength\\Experiment12_OtherClassifiers.csv");
//		Experiment1_AR1_Test(50, new OutFile("C:\\Research\\Results\\JoC2012 Runlength\\Experiment1.csv"));
//		Experiment2_AR1_Classification(new OutFile("C:\\Research\\Results\\JoC2012 Runlength\\Experiment2.csv"));
...
//		Experiment7_AR_RLvsFFT_AFC("C:\\Research\\Results\\JoC2012 RunLength\\Experiment7_RLvsFFT_AFC.csv");
 ...
 * 1. Randomly generate model length between minLength and maxLength
...
// 7. Measure accuracy with ARMA fit. Must be above 60% for 100 and above 90% for 1000
	...
	public static double[] AR_Classification(double[][] paras, Classifier[] c, int n,int nosCases){
		double[] acc=new double[6+c.length];
		Instances train,test;
		Instances arTrain,arTest;
		Instances rlTrain,rlTest;
		Instances acfTrain,acfTest;
		...
		//2. Transform to ACF, ARMA and RunLength
		...
		try{
			...
			DTW_kNN dtw;
                        ...
                        dtw.setMaxR(0.1);
                        int p=0;    
			acc[p++]=ClassifierTools.singleTrainTestSplitAccuracy(new kNN(new EuclideanDistance()),acfTrain,acfTest);
			acc[p++]=ClassifierTools.singleTrainTestSplitAccuracy(new kNN(new EuclideanDistance()),arTrain,arTest);
			acc[p++]=ClassifierTools.singleTrainTestSplitAccuracy(new kNN(new EuclideanDistance()),rlTrain,rlTest);
			acc[p++]=ClassifierTools.singleTrainTestSplitAccuracy(new kNN(new GowerDistance(rlTrain)),rlTrain,rlTest);
			acc[p++]=ClassifierTools.singleTrainTestSplitAccuracy(new kNN(new LikelihoodRatioDistance()),rlTrain,rlTest);
			acc[p++]=ClassifierTools.singleTrainTestSplitAccuracy(dtw,rlTrain,rlTest);
			for(int i=0;i<c.length;i++)
              			acc[p++]=ClassifierTools.singleTrainTestSplitAccuracy(c[i],acfTrain,acfTest);
...
//3. Measure classification accuracy on both 	
			...
			of.writeLine(euclidAcc+","+histAcc+","+armaAcc);
		}
...
//Fit Histogram		
	...
	public static void runLengthTest(){
		...
		{
			Instances test=ClassifierTools.loadData(TimeSeriesClassification.path+TimeSeriesClassification.fileNames[i]+"\\"+TimeSeriesClassification.fileNames[i]+"_TEST.arff");
			Instances train=ClassifierTools.loadData(TimeSeriesClassification.path+TimeSeriesClassification.fileNames[i]+"\\"+TimeSeriesClassification.fileNames[i]+"_TRAIN.arff");
...
//FFT: keep only first 25% of terms?		
		...
		train[0]=ClassifierTools.loadData(path+"train"+file+".arff");
		test[0]=ClassifierTools.loadData(path+"test"+file+".arff");
		try{
